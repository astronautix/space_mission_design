// rune with PYTHONHOME=/usr/bin LIBGL_ALWAYS_SOFTWARE=1 scilab -f crocus_power.sce 


disp("stating the script")
exec("loader_celestlab.sce", 2) // mode 2 without line prompts
...
if doPlots then
   ...
   save("results.h5", "pos_ecf", "-append")
end
// =====================================================
...
Moon_dir = M_eci2sat * CL_unitVector(Moon_eci - pos_eci); 
 ...
 if doPlots then
   ...
   save("results.h5", "cjd", 'Sun_dir' , "-append")
...
interv = CL_ev_eclipse(cjd, pos_eci, Sun_eci, typ = "umb");
 ...
 if doPlots then    
   ...
   save("results.h5", "interv", 'Sun_dir' , "-append")
